...
def cumulative_sums(df, groups_of_interest, latest_date):
    ...
    for group_title, group_label in groups_of_interest.items():
        ...
        elif group_title=="care home": 
            out = df.copy().loc[(df[group_label]==group_title)]
            cols = ["sex", "ageband", "ethnicity_6_groups", "dementia"]
        else:    # age groups
            out = df.copy().loc[(df[group_label]==group_title)]
...
def create_summary_stats(df, summarised_data_dict,  formatted_latest_date, savepath, 
                         groups=["80+", "70-79", "care home", "under 70s, not in other eligible groups shown"],
                        suffix=""):
    """
...
def plot_dem_charts(summary_stats_results, cumulative_data_dict, formatted_latest_date, savepath, savepath_figure_csvs=None, include_overall=False, org_name="", suffix=""):
