...
input_df_params = [
    #patient 2 has left. none have joined
    {
        "obs": {
            ...
            "age_prev_month": pd.Series([20, 40, 50, 60]),
    ...
    # patient 6 has joined. Patient 7 has joined (but because they fit age criteria). none have left
    {
        "obs": {
            ...
            "age_prev_month": pd.Series([20, 30, 40, 50, 60, 70, 17]),
    ...
    return pd.DataFrame(
        {
            ...
            "age_prev_month": pd.Series([20, 30, 40, 50, 60]),
...
class TestGetMovedPatients:
    def test_get_patients_joined_tpp(self, input_df_params, input_df_comparator):
        obs = utilities.get_patients_joined_tpp(pd.DataFrame(input_df_params["obs"]), input_df_comparator, "age", "age_prev_month", ["ethnicity"])
    ...
    def test_get_patients_left_tpp(self,input_df_params, input_df_comparator):
        obs = utilities.get_patients_left_tpp(pd.DataFrame(input_df_params["obs"]), input_df_comparator, "died", ["ethnicity"])
